...
async function loadSpotify(){const box=$('#sp-section');if(!box)return;box.textContent='';
  ...
  if(!st.configured){const h=el('div','tv-empty');h.innerHTML=TR('mus.helpConfig').replace('{uri}','<code>'+esc(st.redirect_uri)+'</code>');box.appendChild(h);return;}
