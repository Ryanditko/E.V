...
async function runCmd(cmd,btn,e){const nm=cmd.trim().replace(/^\//,'').split(/\s+/)[0].toLowerCase();
  ...
  try{const r=await fetch('/api/cmd',{method:'POST',headers:H(),body:JSON.stringify({command:cmd,thread})});
    ...
    loadPanel();}catch(err){p.remove();sys(TR('err.prefix')+err);}finally{setState();}}
f.onsubmit=e=>{e.preventDefault();if(slash.style.display==='block'&&slSel>=0){pickSlash();return;}
  ...
  if(m.startsWith('/')){const raw=m.slice(1).trim().toLowerCase();
    if(raw==='modo'||raw.startsWith('modo ')){
      ...
      sys(on?TR('focus.sysOn'):TR('focus.sysOff'));return;}
    runCmd(m.slice(1));}
  else send(m);};
...
function applySerious(on,announce){on=!!on;document.body.classList.toggle('serious',on);
  ...
  try{if(window.speak)speak(on?TR('focus.sayOn'):TR('focus.sayOff'));}catch(e){}}
...
async function openConnectors(){
  ...
  const drawKeys=()=>{klist.textContent='';if(!keys.length)klist.appendChild(el('div','tv-empty',TR('keys.empty')));
  ...
  const drawConns=()=>{clist.textContent='';if(!conns.length)clist.appendChild(el('div','tv-empty',TR('conns.empty')));
...
function openPageBuilder(page){const m=$('#modal');m.textContent='';const card=el('div','mcard');
  ...
  const list=el('div','vlist');
  const draw=()=>{list.textContent='';if(!widgets.length)list.appendChild(el('div','tv-empty',TR('widgets.empty')));
  ...
  const arg=document.createElement('input');arg.className='minput';arg.placeholder=TR('widgets.argPh');card.appendChild(arg);
  const add=el('button','mbtn2',TR('widgets.add'));add.style.marginTop='6px';add.onclick=()=>{const t=sel.value,a=(arg.value||'').trim();const w={type:t};
...
function ptick(){if(!pomo.run)return;pomo.rem--;
  if(pomo.rem<=0){
    if(pomo.phase==='focus'){pomo.phase='break';pomo.total=pomo.brk;pomo.rem=pomo.brk;PL.textContent=t('pomo.break');speak(TR('pomo.sayBreak'),true);}
    else{pomo.rem=0;pstop();PL.textContent=t('pomo.cycleDone');speak(TR('pomo.sayCycle'),true);}
  }prender();}
...
function openPomo(mins,brk){mins=mins||25;brk=brk||5;pomo.phase='focus';pomo.brk=brk*60;pomo.total=mins*60;pomo.rem=mins*60;PL.textContent=t('pomo.focus');PW.classList.add('on');pstart();speak(TR('pomo.sayStartPre')+mins+TR('pomo.sayStartPost'),true);}
...
function audioConfirm(blob){const p=$('#audprev');p.innerHTML='';if(!blob||!blob.size){p.style.display='none';return;}
  ...
  const play=el('button','ap-play',TR('ap.listen'));play.onclick=()=>{try{_audA.currentTime=0;_audA.play();}catch(e){}};
  const info=el('span','ap-info',TR('vc.audioRecorded'));
  const cancel=el('button','ap-cancel',t('common.cancel'));cancel.onclick=()=>{p.style.display='none';if(_audA){try{_audA.pause();}catch(e){}}};
  const sendb=el('button','ap-send',TR('chat.send'));sendb.onclick=async()=>{p.style.display='none';setState('thinking');
...
vcMic.onclick=async()=>{
  ...
  if(_recActive){stopRec();vcTxt.textContent=TR('msg.transcribing');return;}
...
function ensureRec(){if(_rec)return true;if(!SR)return false;
  ...
  _rec.onresult=ev=>{for(let i=ev.resultIndex;i<ev.results.length;i++){const res=ev.results[i];
    ...
    if(_convo){                             // conversa contínua: sem palavra-mágica
      ...
      if(/^(parar|encerrar|tchau|chega|para)\b/i.test(t)){toggleConvo(false);speak(TR('vc.convoEnd'),true);continue;}
    ...
    if(!cmd){hfSay(TR('vc.attn'));speak(TR('vc.attnShort'),true);continue;}
    processHF(cmd);}};
  _rec.onerror=e=>{if(e.error==='not-allowed'||e.error==='service-not-allowed'){
    _hf=false;_ambient=false;renderHFBtn();renderAmbBtn();hfSay(TR('vc.micPermit'));}};
...
function renderHFBtn(){const b=$('#vc-cont');b.innerHTML='';b.appendChild(ficon(_hf?'ear':'ear-off'));
  b.appendChild(document.createTextNode(' '+TR('vc.handsFreeLbl')+(_hf?TR('vc.hfOn'):TR('vc.hfOff'))));b.classList.toggle('on',_hf);window.lucide&&lucide.createIcons();}
...
function toggleConvo(on){const want=(on===undefined)?!_convo:on;
  if(want&&!SR){vcTxt.textContent=TR('vc.needBrowser');return;}
  _convo=want;
...
async function loadInicio(){
  ...
  [[t('ov.sgSummary'),t('send.summary')],[t('ov.sgToday'),t('send.today')],[t('ov.sgExpenses'),t('send.expenses')],[t('ov.sgWeather'),t('send.weather')]].forEach(s=>{const b=el('button');b.textContent=s[0];b.onclick=()=>{switchView('chat');send(s[1]);};sug.appendChild(b);});
  ...
  qb('sunrise',t('cat.plano'),()=>{switchView('chat');send(t('send.plan'));});
  qb('bell-ring',t('cat.pendencias'),()=>{switchView('chat');send(t('send.pending'));});
  qb('timer',t('cat.foco'),()=>{openPomo(25);});
  qb('newspaper',t('cat.noticias'),()=>{switchView('chat');send(t('send.news'));});
...
async function loadCofre(){const box=$('#cf-list');if(!box)return;box.textContent='';
  ...
  if(!items.length){box.appendChild(q?el('div','tv-empty',t('vault.notFound')):emptyState('folder-lock',t('empty.vaultT'),t('empty.vaultH')));window.lucide&&lucide.createIcons();return;}
...
(function(){const f=$('#cf-file');if(f)f.onchange=async e=>{const file=e.target.files[0];if(!file)return;const fd=new FormData();fd.append('file',file);
  toast(TR('msg.sending'));try{const r=await fetch('/api/vault',{method:'POST',headers:{'Authorization':'Bearer '+token},body:fd});if(r.ok){sfx('confirm');loadCofre();}else{const j=await r.json().catch(()=>({}));toast(j.detail||TR('msg.failed'));}}catch(_){toast(TR('msg.failed'));}e.target.value='';};
...
async function loadPainel(){const A=$('#pd-astro'),R=$('#pd-radar');if(!A||!R)return;
  ...
  loadBackupStatus();
  const zones=[[TR('clock.saopaulo'),'America/Sao_Paulo'],[TR('clock.newyork'),'America/New_York'],[TR('clock.london'),'Europe/London'],[TR('clock.tokyo'),'Asia/Tokyo']];
...
async function loadSpotify(){const box=$('#sp-section');if(!box)return;box.textContent='';
  ...
  if(!st.configured){const h=el('div','tv-empty');h.innerHTML=TR('spotify.setup1')+'<code>'+esc(st.redirect_uri)+'</code>'+TR('spotify.setup2');box.appendChild(h);return;}
  ...
  const si=document.createElement('input');si.className='tv-search';si.placeholder=TR('spotify.searchPh');si.style.flex='1';
...
function askEV(name,lat,lng){switchView('chat');send(TR('send.about').replace('{name}',name));}
...
function addSavedMarker(p){if(!_savedLayer)_savedLayer=L.layerGroup().addTo(_map);const m=L.marker([p.lat,p.lng]).addTo(_savedLayer);_savedMarkers[p.id]=m;
  m.bindPopup(()=>{const d=document.createElement('div');d.innerHTML='<div class="pop-n">'+esc(p.name)+'</div><div class="pop-d">'+TR('map.savedPoint')+'</div>';
...
function locateMe(){const st=$('#map-status');if(!navigator.geolocation){st.textContent=TR('map.geoUnavail');return;}
  st.textContent=TR('map.locatingDevice');
...
function loadMap(){
  ...
  if(!_map){_map=L.map('map',{zoomControl:false,attributionControl:false}).setView([-23.5505,-46.6333],12);
    ...
    $('#map-ask').onclick=()=>{switchView('chat');send(TR('send.nearby'));};
...
async function loadSub(){try{const items=(await (await fetch('/api/recurring',{headers:H()})).json()).items||[];const box=$('#sublist');box.textContent='';
  ...
  items.forEach(x=>{const row=el('div','tv-row');const t=el('div','txt');t.appendChild(el('div','',x.description));t.appendChild(subline(x.category+' · '+TR('sub.dayWord')+' '+x.day));
...
async function loadOrc(){try{const items=(await (await fetch('/api/budgets',{headers:H()})).json()).items||[];const box=$('#orclist');box.textContent='';
  ...
  items.forEach(b=>{const row=el('div','tv-row');const t=el('div','txt');t.appendChild(el('div','',b.category));
    const val=el('div','');val.style.cssText='font-family:var(--mono);font-weight:600';val.textContent='R$'+b.amount.toFixed(0)+TR('budget.perMonth');
...
async function loadMon(){try{const items=(await (await fetch('/api/watches',{headers:H()})).json()).items||[];const box=$('#monlist');box.textContent='';
  ...
  items.forEach(w=>{const row=el('div','tv-row');const t=el('div','txt');const a=document.createElement('a');a.href=w.url;a.target='_blank';a.rel='noopener';a.className='lnk';a.textContent=w.url;t.appendChild(a);if(w.keyword)t.appendChild(subline(TR('sub.keywordWord')+': '+w.keyword));
...
$('#kb-urlf').onsubmit=e=>{e.preventDefault();const url=$('#kb-url').value.trim();if(!url)return;
  ...
  openForm(t('form.indexPage'),[{key:'name',label:t('field.nameId'),value:def},{key:'url',label:t('field.url'),value:url}],async v=>{
    if(!v.url)return;$('#kb-fmsg').textContent=TR('msg.indexing');
...
$('#kb-file').onchange=e=>{const f=e.target.files[0];if(!f)return;e.target.value='';
  openForm(t('form.nameFile'),[{key:'name',label:t('field.nameId'),value:f.name}],async v=>{
    $('#kb-fmsg').textContent=TR('msg.sending');const fd=new FormData();fd.append('file',f);if(v.name)fd.append('title',v.name);
...
function welcome(){const w=$('#welcome'),txt=$('#welcome-txt');
  if(txt)txt.textContent=TR('greeting.welcome');
...
function showStandby(){const st=$('#standby');if(!st||st.classList.contains('on')||sbBusy())return;
  ...
  if(ss)ss.textContent=(t||l)?(_lang==='pt'?('Você tem '+t+' tarefa'+(t!=1?'s':'')+(l?(' e '+l+' lembrete'+(l!=1?'s':'')):'')+' pra hoje, Ryan.'):('You have '+t+' task'+(t!=1?'s':'')+(l?(' and '+l+' reminder'+(l!=1?'s':'')):'')+' for today, Ryan.')):(_lang==='pt'?'Tudo tranquilo por aqui, Ryan.':'All quiet here, Ryan.');
...
async function loadBrain(){
  ...
  if(!brainLoaded){
    ...
    const cnt=$('#brain-count');if(cnt)cnt.textContent=Math.max(0,(d.nodes||[]).length-1)+' '+TR('brain.neurons')+' · '+(d.links||[]).length+' '+TR('brain.synapses');
  }
  brainResize();
...
async function qrLoop(){if(!_qrScan)return;const v=$('#cam-video');
  if(v&&v.videoWidth){try{const codes=await _qrDet.detect(v);if(codes&&codes.length){const val=(codes[0].rawValue||'').trim();stopQR();
    ...
    else if(val){camResult(TR('cam.codeRead')+val);speak(TR('cam.saidCode')+val,true);}else camResult(TR('cam.cantRead'));return;}}catch(e){}}
...
async function camFace(){if(_fapiBusy)return;_fapiBusy=true;camResult(TR('cam.faceAnalyzing'));
  try{const desc=await faceDescriptor();
    ...
    if(!st.enrolled){
      ...
      camResult(TR('cam.faceEnrolled'));speak(TR('cam.sayEnrolled'),true);return;}
    const d=faceDist(desc,st.descriptor);
    if(d<0.52){camResult(TR('cam.faceHi'));speak(TR('cam.sayHi'),true);}
